...
$("#random").on("click", function (event) {
    ...
    $.getJSON("./assets/json/alldata.json", function (response) {
        ...
        // iterate all the file and store all the ids
        for (i = 0; i < response.data.length; i++) {

            allLocationIDs.push(response.data[i].id);
        }

        console.log("lenght of the all locations array after being filled: " + allLocationIDs.length);

        // ! logic to selecting an id form the array

        // generating random number from 0 to the all locations lenght
        var randomIndex = Math.floor(Math.random() * allLocationIDs.length);
        var randomID = allLocationIDs[randomIndex];
